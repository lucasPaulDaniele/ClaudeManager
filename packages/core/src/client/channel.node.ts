/**
 * LE CANAL VERS MA FENETRE — resoudre, PUIS confirmer, et jamais l'inverse.
 *
 * ─────────────────────────────────────────────────────────────────────────────────────────
 * POURQUOI CE MODULE EXISTE, ET CE N'EST PAS UN RANGEMENT. L'ordre « resoudre la fenetre →
 * CONFIRMER LE CANAL par `GET /health` → seulement alors agir » FAIT PARTIE DU MECANISME
 * (ADR-003, decision 5) : c'est lui qui transforme « on a peut-etre agi sur la mauvaise
 * fenetre » en « on n'a rien fait, et on dit pourquoi ». Il etait ecrit une fois, dans le
 * chemin d'ouverture. L'increment C4 en ajoute DEUX autres — enumerer, fermer —, dont l'un a
 * un effet de bord.
 *
 * Recopier la sequence trois fois, c'est accepter qu'elle diverge une fois : le jour ou l'une
 * des trois cesserait de confirmer, rien ne le signalerait, et l'invariant du produit
 * (principe fondateur n.2) tomberait sur cette seule route. Elle est donc ECRITE UNE FOIS et
 * les trois commandes la traversent — c'est la meme raison qui a fait remonter les libelles de
 * route dans le coeur plutot que d'en garder une copie dans le serveur.
 * ─────────────────────────────────────────────────────────────────────────────────────────
 *
 * POURQUOI LA CONFIRMATION VAUT AUSSI POUR UNE LECTURE : parce qu'elle ne protege pas l'effet
 * de bord, elle protege l'IDENTITE. Une enumeration servie par une autre fenetre que la mienne
 * rendrait des poignees qui ne designent rien chez moi — et un agent les passerait ensuite a
 * une fermeture. Un renseignement faux se paie au coup suivant.
 *
...
/** Ce que toute commande du client demande au monde exterieur, et rien de plus. */
...
/** La fenetre resolue, son canal CONFIRME, et ce que la lecture du registre a ecarte. */
export interface WindowChannel {
  /** L'entree BRUTE — port et jeton en clair. Elle ne sort jamais telle quelle. */
...
/**
 * Resout la fenetre du processus appelant, et CONFIRME son canal — dans cet ordre.
 *
 ...
 * `DUPLICATE_WINDOW_IDENTITY`, puis toutes celles de la confirmation.
 */
export async function openWindowChannel(options: WindowChannelOptions): Promise<WindowChannel> {
  ...
  const entry = requireOwningWindow(options.pid, options.snapshot.table, registry.windows);

  return { entry, channel: await confirmChannel(entry, options.transport), skipped: registry.skipped };
}
