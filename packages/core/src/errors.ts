...
export const ERROR_CODES = {
   ...
   *
   * CE CODE EST RECENT, ET NE JAMAIS L'OUBLIER EN LE DOCUMENTANT : il n'existe qu'a partir de
   * la correction du gate de mi-lot C. Les CLI 0.2.0 et 0.3.0 ne le portent PAS — verifie, zero
   * occurrence — et rangent le meme refus sous WINDOW_RESPONSE_UNREADABLE, avec la remediation
   * des routes de lecture. Une doc qui attribue ce code a une CLI d'epoque envoie donc son
   * lecteur vers une conduite qui n'est pas celle qu'il lira.
   */
...
const REMEDIATIONS: Readonly<Record<ErrorCode, string>> = {
  ...
  [ERROR_CODES.WINDOW_RESPONSE_UNREADABLE]:
    "La fenetre hote a repondu, mais sa reponse n'est pas de la forme attendue. RELANCER EST SUR, et c'est la seule chose qui distingue ce code de WINDOW_OPEN_RESPONSE_UNREADABLE : il ne tombe que sur des routes dont une seconde demande ne peut RIEN creer. Les routes de lecture (GET /health, cmgr conversations) n'ont aucun effet de bord du tout ; la FERMETURE en a un, et elle est neanmoins ici, parce qu'un second appel sur la MEME poignee ne peut RIEN fermer : la poignee est DEPENSEE des que l'editeur a ete sollicite, et une seconde demande sort en CONVERSATION_ALREADY_CLOSED ou en CONVERSATION_HANDLE_STALE sans toucher a un seul onglet. La version de l'extension compagnon installee dans cette fenetre ne parle probablement pas le meme protocole que cette CLI : comparer son extensionVersion avec `cmgr windows`, mettre les deux artefacts a jour ensemble, puis OUVRIR UNE FENETRE NEUVE. NE PAS RECHARGER une fenetre qui porte une conversation : une fenetre deja ouverte ne prend jamais une version nouvelle, meme rechargee, et un rechargement tue les claude.exe qui descendent de son extension host (docs/adr/005-activation-de-l-extension-compagnon.md). LA COMPARAISON DES NUMEROS NE CONCLUT PAS TOUJOURS : pendant le lot C le protocole a change SANS que le numero monte — l'extension 0.2.0 et la CLI 0.3.0 designent chacune DEUX etats de protocole. Depuis, toute evolution observable du protocole monte le numero dans l'increment qui la produit. Les details portent la route et ce qui manquait.",
  [ERROR_CODES.WINDOW_OPEN_RESPONSE_UNREADABLE]:
    "La fenetre hote a repondu a la DEMANDE D'OUVERTURE, mais sa reponse n'est pas de la forme attendue. UNE CONVERSATION A PEUT-ETRE ETE OUVERTE, ET LE TOUR 1 JOUE : cette validation est posterieure a l'effet de bord, contrairement a WINDOW_RESPONSE_UNREADABLE. NE PAS RELANCER A L'AVEUGLE — une seconde demande ouvrirait une seconde conversation par-dessus la premiere. Constater l'etat reel dans la fenetre elle-meme (l'onglet de conversation y est visible) ; `cmgr conversations` le dit sans regarder l'ecran, et sa relance est sure — c'est une route de lecture. Cause la plus probable : la version de l'extension compagnon installee dans cette fenetre ne parle pas le meme protocole que cette CLI — comparer son extensionVersion avec `cmgr windows`, mettre les deux artefacts a jour ensemble, puis ouvrir une fenetre NEUVE. NE PAS recharger celle-ci : un rechargement tue les claude.exe qui descendent de son extension host, donc la conversation qui vient peut-etre de s'ouvrir. LA COMPARAISON DES NUMEROS NE CONCLUT PAS TOUJOURS : pendant le lot C le protocole a change SANS que le numero monte — l'extension 0.2.0 et la CLI 0.3.0 designent chacune DEUX etats de protocole. Depuis, toute evolution observable du protocole monte le numero dans l'increment qui la produit.",
  ...
  [ERROR_CODES.CONVERSATION_HANDLE_STALE]:
    "La fenetre ne peut pas prouver que l'onglet designe est celui qui avait ete liste. AUCUN ONGLET N'A ETE FERME, et c'est la garantie du produit : la fermeture exige une preuve d'identite plutot que de fermer au plus probable. TROIS ETATS L'ENTRAINENT : la poignee n'a jamais ete emise par cette fenetre (elle vient d'ailleurs, ou l'extension host a redemarre depuis — les poignees n'y survivent pas) ; l'onglet designe ne correspond plus a ce qui avait ete releve (son libelle a change, ou il a bouge) ; ou LES ONGLETS DE CONVERSATION DE LA FENETRE NE SONT PLUS DISPOSES COMME AU MOMENT DE LISTER — une conversation a paru, disparu ou change de place depuis. Ce dernier cas est le plus frequent, et il est VOULU : aucun onglet Claude ne porte d'identifiant stable (docs/compatibilite.md, D2 et D24), une poignee ne designe donc pas un onglet mais UNE PLACE DANS UN ARRANGEMENT, et l'arrangement a bouge. Une poignee DEJA EMPLOYEE pour fermer ne ferme jamais une seconde fois : elle sort ici aussi. LE GESTE, EN DEUX TEMPS ET DANS CET ORDRE : relancer `cmgr conversations`, puis REGARDER si la conversation qu'on voulait fermer y figure encore. Si oui, retenter avec sa poignee fraiche. SI NON, ELLE EST DEJA FERMEE : ne pas fermer celle qui a pris sa place. Ce code ne dit PAS que l'onglet existe encore — il dit que la fenetre ne peut pas l'affirmer, et c'est mesure le 2026-07-27 : fermer un onglet fait glisser ses voisins d'un rang, et un onglet dont le libelle change sur place est alors indiscernable d'un onglet parti dont le voisin a pris la place.",
  [ERROR_CODES.CONVERSATION_ALREADY_CLOSED]:
    "Il n'y a rien a fermer. DEUX ETATS L'ENTRAINENT, et tous deux sont des preuves POSITIVES : cette poignee a DEJA ferme sa conversation — la fenetre a constate la disparition de l'onglet, et elle s'en souvient —, ou plus aucun onglet de conversation ne correspond a ce qu'elle avait releve. NE PAS RETENTER — relister n'y changerait rien, c'est la difference exacte avec CONVERSATION_HANDLE_STALE. `cmgr conversations` enumere ce qui reste ouvert dans cette fenetre ; une liste vide n'est pas une erreur.",
  [ERROR_CODES.CONVERSATION_CLOSE_FAILED]:
    "L'editeur a recu la demande de fermeture, et la disparition de l'onglet n'a PAS ete constatee dans le delai accorde. L'absence d'erreur ne prouve jamais la fermeture : c'est l'enumeration qui fait foi. Causes connues, dans cet ordre : l'editeur retient l'onglet (une invite de sauvegarde, un panneau epingle), il n'a jamais repondu du tout (le detail porte alors `editorAnswered: false`), ou il a refuse la fermeture sans le dire. RELANCER AVEC LA MEME POIGNEE NE FERMERA RIEN, et c'est delibere : des que l'editeur a ete sollicite, la poignee est DEPENSEE — une seconde demande sort en CONVERSATION_HANDLE_STALE sans toucher a un seul onglet. C'est ce qui rend la reprise sure au lieu de la rendre dangereuse : avant la correction du gate final du lot C, une relance pouvait fermer la conversation du VOISIN qui avait glisse sur la place liberee. LE GESTE, EN DEUX TEMPS : `cmgr conversations`, qui dit ce que la fenetre porte encore ; puis, si la conversation visee y figure toujours, la fermer avec sa poignee FRAICHE. UN CAS OU CE CODE TOMBE SUR UNE FERMETURE POURTANT REUSSIE, et il vaut mieux le savoir que le decouvrir : la confirmation exige que le NOMBRE d'onglets de conversation ait diminue, et une conversation qui s'OUVRE dans la meme fenetre pendant l'attente le ramene a son compte de depart. Les details discriminent — `conversationsAfter` egal a `conversationsBefore` peut venir de l'une ou l'autre cause, un compte plus bas designe la premiere. Ce compromis est assume : il ferme un FAUX SUCCES, qui serait bien plus couteux (voir packages/vscode/src/tabs.ts, `removalConfirmed`).",
};
