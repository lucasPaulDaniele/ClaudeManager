...
- **Couverture** : `@vitest/coverage-v8`, deux seuils **configurés et vérifiés** dans `vitest.config.ts` — **100 %** sur `packages/core/src/**`, et un seuil global fixé **au plancher entier de ce qui est réellement atteint** (99 % lignes et instructions, 98 % branches, 98 % fonctions — mesure du 2026-07-27, correction du gate C volet 2 : 99,53 · 99,04 · 98,75 · 99,53, identique sous Windows et sous Linux). Jamais un chiffre d'intention : un seuil qu'on n'atteint pas est un seuil qu'on désactivera. Le seuil se relève quand la couverture monte ; il ne s'abaisse **jamais en silence** — la seule soupape est une justification nommée et datée. **Une exception, nommée et datée le 2026-07-27** : les branches restent à 98 pour un réel de 99,04, parce que le compte absolu ne suit pas le pourcentage — 832 branches couvertes sur 840, quand un seuil à 99 en exige 832. La marge est de **zéro branche** ; un plancher exact mais intenable se contourne au premier incident, et c'est alors la règle entière qui perd son autorité. Le détail du calcul est dans `vitest.config.ts`, avec la localisation des huit branches restantes. Y figurent aussi **les exclusions nommées et datées, énumérées dans `vitest.config.ts`** *(pas de cardinal ici : il serait faux au prochain ajout)*, parce que l'API de l'éditeur en est la substance même et que `npm run test:integration` s'en charge. Toute nouvelle exclusion se justifie au même endroit, avec sa date.
